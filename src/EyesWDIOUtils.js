...
(function () {
    ...
    return promiseFactory.makePromise(function (resolve, reject) {
      return EyesWDIOUtils.executeScript(browser, JS_GET_VIEWPORT_SIZE, promiseFactory, undefined).then(function (results) {
        if (isNaN(results.value[0]) || isNaN(results.value[1])) {
          ...
          resolve({
            width: parseInt(results.value[0], 10) || 0,
            height: parseInt(results.value[1], 10) || 0
          });
        }
      }, function (err) {
        reject(err);
  ...
  EyesWDIOUtils.getViewportSizeOrDisplaySize = function (logger, browser, promiseFactory) {
    ...
    return EyesWDIOUtils.getViewportSize(browser, promiseFactory).catch(function (err) {
      ...
      return browser.windowHandleSize().then(function (size) {
  ...
  function _setBrowserSize(logger, browser, requiredSize, retries, promiseFactory) {
    return promiseFactory.makePromise(function (resolve, reject) {
      ...
      return browser.windowHandleSize({width: requiredSize.width, height: requiredSize.height}).then(function () {
        return GeneralUtils.sleep(1000, promiseFactory);
      }).then(function () {
        return browser.windowHandleSize();
      }).then(function (currentSize) {
        logger.log("Current browser size:", currentSize);
        if (currentSize.value.width === requiredSize.width && currentSize.value.height === requiredSize.height) {
          resolve();
          return;
        }
    ...
    return promiseFactory.makePromise(function (resolve, reject) {
      try {
        ...
        EyesWDIOUtils.getViewportSize(browser, promiseFactory).then(function (viewportSize) {
          ...
          browser.windowHandlePosition({x: 0, y: 0}).catch(function () {
            logger.verbose("Warning: Failed to move the browser window to (0,0)");
          }).then(function () {
            return EyesWDIOUtils.setBrowserSizeByViewportSize(logger, browser, actualViewportSize, requiredSize, promiseFactory);
          }).then(function () {
            ...
            return EyesWDIOUtils.setBrowserSizeByViewportSize(logger, browser, actualViewportSize, requiredSize, promiseFactory).then(function () {
              return EyesWDIOUtils.getViewportSize(browser, promiseFactory);
  ...
  function _setWindowSize(logger,
    ...
    return promiseFactory.makePromise(function (resolve, reject) {
      ...
      return EyesWDIOUtils.setBrowserSize(logger, browser, requiredBrowserSize, promiseFactory).then(function () {
  ...
  EyesWDIOUtils.getScreenshot = function getScreenshot(browser,
    ...
    return positionProvider.getEntireSize().then(function (pageSize) {
      ...
      return EyesWDIOUtils.getDevicePixelRatio(browser, promiseFactory).then(function (ratio) {
        pixelRatio = ratio;
      }, function () {
      ...
      if (hideScrollbars) {
        return EyesWDIOUtils.setOverflow(browser, "hidden", promiseFactory).then(function (originalVal) {
      ...
      if (hideScrollbars) {
        return EyesWDIOUtils.setOverflow(browser, originalOverflow, promiseFactory);
      }
    }).then(function () {
      if (originalBodyOverflow) {
        return EyesWDIOUtils.setBodyOverflow(browser, originalBodyOverflow, promiseFactory);
      }
    }).then(function () {
      if (fullPage) {
