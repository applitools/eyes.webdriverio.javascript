...
class EyesTargetLocator {
  ...
  static _framesSetPosition(targetLocator, obj, retries, promiseFactory) {
    return promiseFactory.makePromise(function (resolve) {
      ...
      targetLocator._scrollPosition.setPosition(frame.getParentScrollPosition()).then(function () {
        ...
        targetLocator.controlFlow().then(function () {
          EyesTargetLocator._framesSetPosition(targetLocator, obj, retries - 1, promiseFactory).then(function () {
            resolve();
          });
        });
      });
  ...
  static _framesSetPositionFromArray(targetLocator, obj, retries, promiseFactory) {
    return promiseFactory.makePromise(function (resolve) {
      ...
      targetLocator._driver.switchTo().frame(obj[retries]).then(function () {
        ...
        targetLocator.controlFlow().then(function () {
          EyesTargetLocator._framesSetPositionFromArray(targetLocator, obj, retries - 1, promiseFactory).then(function () {
            resolve();
          });
        });
      });
